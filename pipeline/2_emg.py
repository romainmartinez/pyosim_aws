...
params = {
    ...
    "order": 4,
    "outlier": 5,
    "emg_labels": conf.get_conf_field(
        participant=participants[0], field=["emg", "targets"]
    ),
}
...
for i, iparticipant in enumerate(participants):
    ...
    conf.add_conf_field({iparticipant: {"emg": {"mva": mva}}})
    ...
    def process(trial):
        ...
        for iassign in assigned:
            ...
            try:
                emg = Analogs3dOsim.from_c3d(
                    trial, names=iassign_without_nans, prefix=":"
                )
                if nan_idx:
        ...
        emg = (
            emg.band_pass(
                freq=emg.get_rate,
                order=params["order"],
                cutoff=params["band_pass_cutoff"],
            )
            .center()
            .rectify()
            .low_pass(
        ...
        sto_filename = f"{conf.project_path / iparticipant / '0_emg' / trial.stem}.sto"
